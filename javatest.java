...
private Position<E> mirror(Position<E> p) {
    Node<E> myNode = (Node<E>)p;
    Node<E> newNode = new Node<E>(myNode.getElement());
    Node<E> right = myNode.getRight();
    Node<E> left = myNode.getLeft();

    if (right!=null){
        newNode.setLeft((Node<E>) mirror(right));
    }
    if (left!=null){
        newNode.setRight((Node<E>) mirror(left));
    }
      return newNode;
  }
  ...
  public LinkedBinaryTree<E> mirror(){
        LinkedBinaryTree<E> reversed = new LinkedBinaryTree<E>();
        Node<E> myNode = this.root;
        Node<E> right = myNode.getRight();
        Node<E> left = myNode.getLeft();
        Node<E> newNode = (Node<E>) reversed.addRoot(myNode.getElement());
        if (right!=null){
           newNode.setLeft((Node<E>)mirror(right));
       }
        if (left !=null){
...
/*
	public LinkedBinaryTree<E> mirror() {
		...
		reversed.root = null;
		reversed.root.setElement(this.root.getElement());
	...
	public void copy(Position<E> p, Position<E> q){
		...
		copy(left,addLeft(coRoot,left.getElement()));

		copy(right,addRight(coRoot,right.getElement()));
